...
under `rush` when using the stack.

Standalone ClickHouse uses `rush.clickhouseStandalone.nodeSelector`,
`tolerations`, `affinity`, and `topologySpreadConstraints`. The bundled
operator uses `rush.clickhouse.operator` and
`rush.clickhouse.operator.crdHook`.
